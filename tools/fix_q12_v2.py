...
while i < len(lines):
    ...
    if '"imageOptions": [' in line:
        ...
        q12_check = lines[i+3] if i+3 < len(lines) else ''  # line i+2 is '{', id is on i+3
        ...
        if re.match(r'\s+"[^"]+",\s*$', emoji_line) and '"id"' in q12_check and 'q12' in q12_check:
            ...
            if len(blocks) != 3:
                print(f'WARNING: only found {len(blocks)} blocks at line {i+1}')
            ...
            while j < len(lines) and '}' not in lines[j]:
                j += 1
            # lines[j] is the closing } of q1 object
