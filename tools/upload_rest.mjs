/**
 * LinguaPlay — Firestore Lesson Uploader
 * Firestore rules must be open for lessons_ collections (already done).
 * Run: node tools/upload_rest.mjs
 */

import { initializeApp } from 'firebase/app';
import {
  getFirestore, doc, getDocs, collection, writeBatch
...
// Only JS files (already compiled) — available for 7 languages
const langConfigs = [
  { code: 'en', label: 'English',    file: '../src/data/englishContent.js' },
  { code: 'tr', label: 'Turkish',    file: '../src/data/turkishContent.js' },
  { code: 'fr', label: 'French',     file: '../src/data/frenchContent.js' },
  { code: 'de', label: 'German',     file: '../src/data/germanContent.js' },
  { code: 'it', label: 'Italian',    file: '../src/data/italianContent.js' },
  { code: 'ku', label: 'Kurdish',    file: '../src/data/kurdishContent.js' },
  { code: 'es', label: 'Spanish',    file: '../src/data/spanishContent.js' },
];
...
async function main() {
  ...
  for (const lang of langConfigs) {
    ...
    let content;
    try {
      const mod = await import(lang.file);
      content = mod.default ?? mod[Object.keys(mod).find(k => Array.isArray(mod[k]))];
