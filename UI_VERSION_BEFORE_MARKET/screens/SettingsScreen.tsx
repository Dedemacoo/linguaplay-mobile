    ...
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[styles.sectionGroup, { borderColor: colors.border }]}>
          ...
          {renderItem('Öğrenilen Dil', '📚', showComingSoon, <Text style={[styles.valueText, { color: colors.textLight }]}>İngilizce</Text>)}
        ...
        <View style={[styles.sectionGroup, { borderColor: colors.border }]}>
          {renderItem('Premium Abonelik', '💎', showComingSoon)}
          {renderItem('Satın Alımları Geri Yükle', '🔄', showComingSoon)}
          {renderItem('Premium Avantajları', '⭐', showComingSoon)}
        </View>

        {/* ── LINGO AYARLARI ── */}
        <Text style={[styles.sectionTitle, { color: colors.textLight }]}>🐬 LINGO AYARLARI</Text>
        <View style={[styles.sectionGroup, { borderColor: colors.border }]}>
          {renderItem('Ses', '🎙️', showComingSoon)}
          {renderItem('Animasyonlar', '🎬', showComingSoon)}
          {renderItem('Selamlamalar', '👋', showComingSoon)}
          {renderItem('Kişilik', '🧠', showComingSoon)}
          {renderItem('İfade Tarzı', '🎭', showComingSoon)}
          ...
          {renderItem('Asistan Sesi', '🤖', showComingSoon)}
        </View>

        {/* ── HESAP & GÜVENLİK ── */}
        <Text style={[styles.sectionTitle, { color: colors.textLight }]}>🛡 HESAP VE GÜVENLİK</Text>
        <View style={[styles.sectionGroup, { borderColor: colors.border }]}>
          ...
          {renderItem('İki Faktörlü Doğrulama', '🔑', showComingSoon)}
          {renderItem('Cihazlar', '📱', showComingSoon)}
          {renderItem('Oturumlar', '⏱️', showComingSoon)}
          {renderItem('Tüm Cihazlardan Çıkış Yap', '🚪', handleLogoutAll)}
        </View>

        {/* ── HAKKINDA & DESTEK ── */}
        <Text style={[styles.sectionTitle, { color: colors.textLight }]}>📄 HAKKINDA VE DESTEK</Text>
        <View style={[styles.sectionGroup, { borderColor: colors.border }]}>
          {renderItem('Yardım Merkezi', '❓', () => navigation.navigate('Feedback'))}
          {renderItem('Geri Bildirim Gönder', '💬', () => navigation.navigate('Feedback'))}
          {renderItem('Açık Kaynak Lisansları', '⚖️', showComingSoon)}
          ...
          {renderItem('Sürüm', '📌', () => {}, <Text style={[styles.valueText, { color: colors.textLight }]}>1.0.0</Text>)}
          {renderItem('Build Numarası', '⚙️', () => {}, <Text style={[styles.valueText, { color: colors.textLight }]}>24</Text>)}
          {renderItem('Sunucu Durumu', '🟢', () => {}, <Text style={[styles.valueText, { color: colors.textLight }]}>Çevrimiçi</Text>)}
        </View>
