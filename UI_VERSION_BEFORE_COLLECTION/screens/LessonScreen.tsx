...
import { BRAND, useThemeColors } from '../theme/colors';
...
const MascotBubble: React.FC<{ message: string; emotion?: 'default' | 'celebrate' | 'thinking' | 'sad' | 'excited' }> = ({ message, emotion = 'default' }) => {
  ...
  return (
    <Animated.View style={[styles.mascotContainer, { transform: [{ scale: scaleAnim }] }]}>
      <Mascot size={60} emotion={emotion} animated={true} style={{ marginRight: 10 }} />
      <View style={[styles.speechBubble, { backgroundColor: BRAND.surface }]}>
        <Text style={[styles.speechText, { color: BRAND.text }]}>{message}</Text>
        <View style={[styles.speechTail, { borderTopColor: BRAND.surface }]} />
      </View>
    </Animated.View>
  );
};
...
const ComboBadge: React.FC<{ combo: number; colors: any }> = ({ combo, colors }) => {
  ...
  const comboColor = combo >= 4 ? '#FF9600' : combo >= 3 ? '#FFC800' : BRAND.primary;
...
const NeonTypewriter = ({ text, onComplete }: { text: string, onComplete?: () => void }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      Animated.sequence([
        Animated.timing(pulseAnim, { toValue: 1.2, duration: 50, useNativeDriver: true }),
  ...
  return (
    <Animated.Text style={[
      styles.firstSentenceText,
      { 
        transform: [{ scale: pulseAnim }],
        color: '#00FFA3',
        textShadowColor: 'rgba(0, 255, 163, 0.8)',
...
const LessonScreen: React.FC<Props> = ({ navigation, route }) => {
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: BRAND.bg }]}>
        <View style={styles.emptyState}>
          <Text style={{ color: BRAND.text, fontSize: 18 }}>Bu ders henüz hazır değil.</Text>
          <TouchableOpacity onPress={() => navigation.canGoBack() ? navigation.goBack() : navigation.navigate('MainTabs')} style={[styles.backButton, { backgroundColor: BRAND.primary }]}>
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: BRAND.bg }]}>
        <View style={styles.emptyState}>
          ...
          <Text style={{ color: BRAND.text, fontSize: 24, fontWeight: 'bold', fontFamily: 'SpaceGrotesk_700Bold', marginBottom: 15 }}>Ders Kilitli</Text>
          <Text style={{ color: BRAND.textSub, fontSize: 16, textAlign: 'center', paddingHorizontal: 40, lineHeight: 24 }}>
            Bu derse girebilmek için önce önceki dersleri tamamlamalısın.
          </Text>
          <TouchableOpacity onPress={() => navigation.canGoBack() ? navigation.goBack() : navigation.navigate('MainTabs')} style={[styles.backButton, { backgroundColor: BRAND.primary, marginTop: 30 }]}>
            <Text style={{ color: '#FFF', fontSize: 16, fontWeight: 'bold', fontFamily: 'SpaceGrotesk_700Bold' }}>GERİ DÖN</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
      ...
      return (
        <SafeAreaView style={[styles.container, { backgroundColor: BRAND.bg }]}>
          ...
          <View style={styles.completeContainer}>
            {/* Mascot */}
            <MascotBubble message={placementMessage} emotion="excited" />

            <Text style={[styles.completeTitle, { color: BRAND.text }]}>Seviye Belirleme Tamamlandı!</Text>
            ...
            <View style={[styles.rewardCard, { 
              ...
              <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Belirlenen Seviye</Text>
            </View>

            {/* İstatistik kartları */}
            <View style={styles.rewardContainer}>
              <View style={[styles.rewardCard, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}>
                <Text style={styles.rewardEmoji}>✅</Text>
                <Text style={[styles.rewardValue, { color: BRAND.primary }]}>{correctAnswersCount}/{totalQuestions}</Text>
                <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Doğru Cevap</Text>
              </View>
              <View style={[styles.rewardCard, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}>
                <Text style={styles.rewardEmoji}>📊</Text>
                <Text style={[styles.rewardValue, { color: placementColor }]}>%{placementPct}</Text>
                <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Başarı Oranı</Text>
              </View>
              <View style={[styles.rewardCard, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}>
                <Text style={styles.rewardEmoji}>🔓</Text>
                <Text style={[styles.rewardValue, { color: '#58CC02' }]}>{Math.min(correctAnswersCount, allLessons.length)}</Text>
                <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Açılan Ders</Text>
              </View>
            </View>

            <TouchableOpacity
              style={[styles.continueBtn, { backgroundColor: BRAND.primary, borderBottomColor: BRAND.primaryDark }]}
              onPress={() => {
                if (fromOnboarding) {
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: BRAND.bg }]}>
        ...
        <Modal visible={showLevelUpModal} transparent animationType="fade">
          <View style={styles.modalContainer}>
            <View style={[styles.modalContent, { backgroundColor: BRAND.surface }]}>
              <Text style={styles.modalEmoji}>⭐</Text>
              <Text style={[styles.modalTitle, { color: BRAND.text }]}>Seviye Atladın!</Text>
              <Text style={[styles.modalText, { color: BRAND.primary, fontSize: 24, fontWeight: 'bold', fontFamily: 'SpaceGrotesk_700Bold' }]}>
        ...
        <View style={styles.completeContainer}>
          <MascotBubble message="Harika gidiyorsun! Aynen böyle devam! 🌟" emotion="celebrate" />
          <Text style={[styles.completeTitle, { color: BRAND.text }]}>Ders Tamamlandı!</Text>

          <View style={styles.rewardContainer}>
            <View style={[styles.rewardCard, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}>
              <Text style={styles.rewardEmoji}>🎯</Text>
              <Text style={[styles.rewardValue, { color: BRAND.primary }]}>
                {`${totalQuestions > 0 ? Math.round((correctAnswersCount / totalQuestions) * 100) : 0}%`}
              </Text>
              <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Doğruluk</Text>
            </View>
            <View style={[styles.rewardCard, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}>
              <Text style={styles.rewardEmoji}>🔥</Text>
              <Text style={[styles.rewardValue, { color: '#FF9600' }]}>x{maxCombo}</Text>
              <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Maks Kombo</Text>
            </View>
            <View style={[styles.rewardCard, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}>
              <Text style={styles.rewardEmoji}>❤️</Text>
              <Text style={[styles.rewardValue, { color: BRAND.danger }]}>{hearts}</Text>
              <Text style={[styles.rewardLabel, { color: BRAND.textSub }]}>Kalan Can</Text>
            </View>
          </View>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: BRAND.bg }]}>
      ...
      {/* Header with Combo */}
      <View style={[styles.header, { backgroundColor: BRAND.surface }]}>
        <TouchableOpacity 
        ...
        >
          <Text style={[styles.closeBtn, { color: BRAND.textSub }]}>{isPlacementTest ? '←' : '✖'}</Text>
        </TouchableOpacity>
        <View style={styles.progressCol}>
          <ComboBadge combo={combo} colors={colors} />
          <View style={[styles.progressBarContainer, { backgroundColor: BRAND.border }]}>
            <Animated.View
              style={[
      ...
      <Modal visible={showHeartModal} transparent animationType="slide">
        <View style={styles.modalContainer}>
          <View style={[styles.modalContent, { backgroundColor: BRAND.surface }]}>
            <Text style={styles.modalEmoji}>💔</Text>
            <Text style={[styles.modalTitle, { color: BRAND.text }]}>Canın Bitti!</Text>
            <Text style={[styles.modalText, { color: BRAND.textSub }]}>
            ...
            <TouchableOpacity
              style={[styles.modalBtnOutline, { borderColor: BRAND.textSub }]}
              onPress={() => {
                setShowHeartModal(false);
                navigation.canGoBack() ? navigation.goBack() : navigation.navigate('MainTabs');
              }}
            >
              <Text style={[styles.modalBtnOutlineText, { color: BRAND.textSub }]}>Dersi Kapat</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>
      ...
      {showMascot && (
        <View style={styles.mascotOverlay}>
          <MascotBubble message={mascotMessage} emotion={isCorrect ? 'celebrate' : (isChecked && !isCorrect ? 'sad' : 'happy')} />
        </View>
      )}
      ...
      <Animated.View style={[styles.content, { opacity: fadeAnim, transform: [{ translateX: shakeAnim }, { scale: scaleAnim }] }]}>
        {currentQuestion.type === 'flashcard' ? (
          <View style={{ flex: 1, justifyContent: 'center', alignItems: 'center' }}>
            <Text style={[styles.questionTitle, { color: BRAND.text, textAlign: 'center', marginBottom: 20 }]}>Yeni Kelime Öğren</Text>
            <LinearGradient 
            ...
            >
              ...
              <Text style={{fontSize: 40, fontWeight: '900', color: BRAND.primary, marginBottom: 8, letterSpacing: 1}}>{currentQuestion.audioText}</Text>
              <Text style={{fontSize: 22, fontWeight: '600', color: BRAND.textSub, textAlign: 'center', marginBottom: 30}}>{currentQuestion.prompt}</Text>
              ...
              <Text style={{fontSize: 14, color: BRAND.textSub, marginTop: 15, fontWeight: '500'}}>Sesi dinle ve kelimeyi öğren!</Text>
            </LinearGradient>
          </View>
        ) : currentQuestion.type === 'constructSentence' ? (
          <View style={styles.constructContainer}>
            <Text style={[styles.questionTitle, { color: BRAND.text, marginBottom: 20 }]}>{currentQuestion.prompt}</Text>
            
            <View style={styles.constructHeader}>
              <Text style={{fontSize: 80}}>💜</Text>
              <View style={[styles.constructBubble, { borderColor: BRAND.border, backgroundColor: BRAND.surface }]}>
                 <TouchableOpacity
                  style={[styles.constructSpeakerBtn, { backgroundColor: BRAND.accent }]}
                ...
                <View style={{ flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center' }}>
                  <Text style={{ color: BRAND.text, fontSize: 18, fontWeight: 'bold', fontFamily: 'SpaceGrotesk_700Bold', lineHeight: 26 }}>
                    {currentQuestion.prompt.replace('Şu cümleyi çevir: ', '').replace(/['"]/g, '').trim()}
                  </Text>
                </View>
                <View style={[styles.constructBubbleTail, {borderRightColor: BRAND.border}]} />
                <View style={[styles.constructBubbleTailInner, {borderRightColor: BRAND.surface}]} />
              </View>
            </View>

            <View style={[styles.answerBox, {borderBottomColor: BRAND.border}]}>
               {selectedWords.map((word, idx) => (
                  <TouchableOpacity 
                    key={idx} 
                    style={[styles.wordChip, {backgroundColor: BRAND.surface, borderColor: BRAND.border}]} 
                    onPress={() => !isChecked && setSelectedWords(prev => prev.filter((_, i) => i !== idx))}
                  >
                    <Text style={[styles.wordText, {color: BRAND.text}]}>{word}</Text>
                  </TouchableOpacity>
               ))}
            </View>

            <View style={styles.wordBank}>
               {currentQuestion.options.filter(w => w.trim() !== '').map((word, idx) => {
                  ...
                  return (
                    <TouchableOpacity 
                      ...
                      style={[
                        styles.wordChip, 
                        {
                          backgroundColor: isSelected ? BRAND.border : BRAND.surface, 
                          borderColor: isSelected ? BRAND.border : BRAND.border,
                    ...
                    >
                      <Text style={[styles.wordText, {color: isSelected ? BRAND.border : BRAND.text}]}>{word}</Text>
                    </TouchableOpacity>
                  );
               })}
            </View>
          ...
          <>
            <View style={styles.questionHeader}>
              <Text style={[styles.questionTitle, { color: BRAND.text }]}>{currentQuestion.prompt}</Text>
              <TouchableOpacity 
                style={[styles.speakBtn, { backgroundColor: BRAND.surface, borderColor: BRAND.border }]}
                onPress={() => {
            ...
            <View style={currentQuestion.type === 'imageChoice' ? styles.imageOptionsGrid : styles.options}>
              {currentQuestion.options.map((option, index) => {
                let btnStyle: any = { backgroundColor: BRAND.surface, borderColor: BRAND.border };

                if (isChecked) {
                  if (index === currentQuestion.correctIndex) {
                    btnStyle = { backgroundColor: '#D7FFB8', borderColor: BRAND.primary };
                  } else if (index === selectedIndex && !isCorrect) {
                    btnStyle = { backgroundColor: '#FFDFE0', borderColor: BRAND.danger };
                  }
                } else if (selectedIndex === index) {
                  btnStyle = { backgroundColor: BRAND.accent + '20', borderColor: BRAND.accent };
                }
                ...
                if (currentQuestion.type === 'imageChoice') {
                  return (
                    ...
                    >
                      ...
                      <Text style={[styles.imageOptionText, { color: BRAND.text }]}>{option}</Text>
                ...
                return (
                  <TouchableOpacity
                    ...
                    disabled={isChecked}
                  >
                    <Text style={[styles.optionNumber, { color: BRAND.textSub }]}>{index + 1}</Text>
                    <Text style={[styles.optionText, { color: BRAND.text }]}>{option}</Text>
      ...
      {/* Footer */}
      <View style={[styles.footer, { backgroundColor: BRAND.surface, borderTopColor: BRAND.border }]}>
        {isChecked && (
        ...
        {(currentQuestion?.type === 'constructSentence' || currentQuestion?.type === 'flashcard' || isChecked) && (
          ...
          >
            <LinearGradient
              colors={
                isChecked
                  ? (isCorrect ? [BRAND.primary, '#00B8CC'] : [BRAND.danger, '#CC0044'])
                  : (selectedWords.length > 0 || currentQuestion?.type === 'flashcard' ? [BRAND.primary, '#00B8CC'] : [BRAND.border, BRAND.border])
              }
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 1 }}
              style={[
            ...
            >
              <Text style={[styles.actionBtnText, { color: selectedWords.length > 0 || isChecked || currentQuestion?.type === 'flashcard' ? '#FFF' : BRAND.textSub }]}>
      ...
      {showFirstSentence && (
        <Animated.View style={[styles.firstSentenceOverlay, { backgroundColor: 'rgba(0,0,0,0.85)' }]}>
          <View style={[styles.firstSentenceBadge, { backgroundColor: 'transparent', borderColor: 'transparent', elevation: 0, shadowOpacity: 0 }]}>
            <Text style={{ fontSize: 60, marginBottom: 20 }}>✨</Text>
            <NeonTypewriter text="İLK CÜMLENİ KURDUN!" />
            <Text style={{ color: '#fff', fontSize: 18, marginTop: 20, textAlign: 'center', opacity: 0.9 }}>Harika bir başlangıç yaptın.</Text>
          </View>
        </Animated.View>
      )}
      ...
      <Modal visible={showShareModal} transparent animationType="slide">
        <View style={styles.shareModalBg}>
          <View style={[styles.shareModalContent, { backgroundColor: BRAND.surface }]}>
            <View style={styles.shareModalHeader}>
              <TouchableOpacity onPress={() => setShowShareModal(false)}>
                <Text style={[styles.closeShareBtn, { color: BRAND.textSub }]}>✖</Text>
              </TouchableOpacity>
              <Text style={[styles.shareModalTitle, { color: BRAND.textSub }]}>BU CÜMLEYİ PAYLAŞ</Text>
              <View style={{ width: 20 }} />
            </View>
            
            <View style={[styles.shareCard, { backgroundColor: BRAND.bg, borderColor: BRAND.border }]}>
              <View style={styles.shareFlag}><Text>🇺🇸</Text></View>
              <Text style={[styles.shareTargetText, { color: BRAND.text }]}>{currentQuestion.audioText}</Text>
              <Text style={[styles.shareNativeText, { color: BRAND.textSub }]}>
                {currentQuestion.correctAnswer?.join(' ')}
              </Text>
              <View style={styles.shareMascotRow}>
                <Text style={{color: BRAND.primary, fontWeight: 'bold', fontFamily: 'SpaceGrotesk_700Bold', fontSize: 18}}>Lingum</Text>
            ...
            <View style={styles.shareOptionsRow}>
              {[
                ...
                <View key={i} style={styles.shareOptItem}>
                  ...
                  <Text style={[styles.shareOptLabel, { color: BRAND.text }]}>{opt.label}</Text>
                </View>
              ))}
            </View>
          </View>
...
const styles = StyleSheet.create({
  ...
  shareModalBg: {
    ...
    backgroundColor: 'rgba(11,16,34,0.85)',
  },
  shareModalContent: {
